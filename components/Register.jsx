...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    console.log(
      "🚀 Register: Form submitted, registration type:",
      registrationType
    );
    ...
    try {
      ...
      if (registrationType === "wallet") {
        // Wallet registration using mock API
        data = await mockApi.registerWallet({
          walletAddress: walletAddress,
          userType: userType,
        });
      } else {
        // Form registration using mock API
        data = await mockApi.registerOrganization({
          username,
          organizationName,
          email,
          password,
      ...
      alert(
        `✅ Registration successful! Welcome ${data.username}. Please login to continue.`
      );
      navigate("/login");
    } catch (err) {
